...
public class ToastUtils {
     ...
     * @param context
     */
    public static void showToast_Now(Context context,int resId) {
     ...
     * @param context
     */
    public static void showToast_Now(Context context,String content) {
        final Context mContext = context;
     ...
     * @param context
     */
    public static void showToast_Now(String content, Context context) {
        final Context thiscontext = context;
        final String thiscontent = content;
        ((Activity) context).runOnUiThread(new Runnable() {
            public void run() {
                Toast toast = Toast.makeText(thiscontext, thiscontent, Toast.LENGTH_SHORT);
                toast.setGravity(Gravity.CENTER, 0, 0);
                toast.show();
            }
        });
     ...
     * @param context
     */
    public static void showToast_Now(String content, Context context, int time) {
        final int thistime = time;
        final Context thiscontext = context;
        final String thiscontent = content;
        ((Activity) context).runOnUiThread(new Runnable() {
            public void run() {
                Toast toast = Toast.makeText(thiscontext, thiscontent, Toast.LENGTH_LONG);
                toast.setGravity(Gravity.CENTER, 0, 0);
                toast.show();
            }
        });
    ...
    private static int yOffset         = (int) (64 * InitUtils.getApp().getResources().getDisplayMetrics().density + 0.5);
    ...
    public static void setView(@LayoutRes final int layoutId) {
        LayoutInflater inflate = (LayoutInflater) InitUtils.getApp().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    ...
    private static void show(@StringRes final int resId, final int duration) {
        show(InitUtils.getApp().getResources().getText(resId).toString(), duration);
    }
    ...
    private static void show(@StringRes final int resId, final int duration, final Object... args) {
        show(String.format(InitUtils.getApp().getResources().getString(resId), args), duration);
    }
    ...
    private static void show(final CharSequence text, final int duration) {
        ...
        if (sViewWeakReference != null) {
            final View view = sViewWeakReference.get();
            if (view != null) {
                sToast = new Toast(InitUtils.getApp());
                sToast.setView(view);
        ...
        if (!isCustom) {
            if (messageColor != DEFAULT_COLOR) {
                ...
                sToast = Toast.makeText(InitUtils.getApp(), spannableString, duration);
            } else {
                sToast = Toast.makeText(InitUtils.getApp(), text, duration);
            }
        }
